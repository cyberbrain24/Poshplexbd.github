                    ...
                    {items.map((item: any, idx: number) => (
                      <div key={idx} style={{ textAlign: 'center', minWidth: 100, width: 120 }}>
                        <div style={{ background: '#f5f5f5', borderRadius: 8, padding: 8, marginBottom: 8, width: 120, height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {item.image ? (
                            <Image src={`${(import.meta.env.VITE_SERVER_URL || (window.location.hostname === 'admin.poshplexbd.com' ? 'https://poshplexbd.com' : 'http://localhost:8000'))}${item.image}`} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} alt="product" />
                          ) : (
