...
const CRM_URL =
  (import.meta.env.VITE_SERVER_URL ||
    (window.location.hostname === "admin.poshplexbd.com"
...
const Reviews: React.FC = () => {
  ...
  const [selectedCreateProductId, setSelectedCreateProductId] = useState<number | null>(null);
  ...
  const openCreateModal = async () => {
    ...
    setCreateImageList([]);
    setSelectedCreateProductId(null);
  ...
  const handleCreateSubmit = async (values: any) => {
    const isUploading = createImageList.some(
    ...
    const finalImages = createImageList
      .map((f) => f.url || (f.response && f.response.url))
      .filter(Boolean);
    const token = localStorage.getItem("poshplex_access_token");
    try {
      await axios.post(
        `${CATALOG_URL}/admin/reviews/create`,
        {
          ...values,
          images: finalImages,
        },
      ...
      if (err?.response?.status === 400) {
        message.error(err.response.data.detail || "Failed to create review.");
  ...
  const handleCreateUploadChange = ({ fileList }: any) =>
    setCreateImageList(fileList);
        ...
        renderItem={(item) => (
          <List.Item style={{ height: "100%" }}>
            ...
            >
              ...
              <p style={{ fontSize: 13, flex: 1, margin: "0 0 12px 0" }}>
                {item.comment}
              </p>
              ...
              >
                ...
                >
                  ...
                  <Switch
                    ...
                    onChange={(c) =>
                      handleModerate(item.id, { is_approved: c })
                    }
                  />
                </div>
                <div
                ...
                >
                  ...
                  <Switch
                    ...
                    onChange={(c) =>
                      handleModerate(item.id, { is_featured: c })
                    }
                  />
                </div>
                <Button size="small" block onClick={() => openEditModal(item)}>
                  Edit Review
                </Button>
              </Space>
            </Card>
        ...
        >
          ...
          <Form.Item label="Images">
            <Upload
              action={`${CATALOG_URL}/reviews/upload-photo`}
              headers={{ Authorization: `Bearer ${uploadToken}` }}
              data={{ product_id: selectedCreateProductId }}
              listType="picture-card"
              fileList={createImageList}
              onChange={handleCreateUploadChange}
            >
              {createImageList.length >= 5 ? null : (
                <div>
                  <PlusOutlined />
                  <div style={{ marginTop: 8 }}>Upload</div>
                </div>
      ...
      >
        ...
        >
          <Form.Item name="name" label="Tier Name" rules={[{ required: true }]}>
            <Input
              placeholder="e.g., VIP, Gold, Diamond"
...
export const CRM: React.FC = () => {
  ...
  const downloadCrmTemplate = () => {
    ...
    csvContent +=
      "Name,Phone,Email,Gender,Birthdate,Membership Type,District,Thana,Address,Active\n";
    csvContent +=
      '"Imran Ahmed","01711223344","imran@gmail.com","male","1995-05-12","VIP","Dhaka","Gulshan","House 12, Road 4","Yes"\n';
    csvContent +=
      '"Sadia Islam","01811223344","sadia@gmail.com","female","1998-08-20","Regular","Chittagong","Kotwali","Flat 3A, Building B","Yes"\n';
  ...
  const handleCsvUpload = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      message.success(
        "CSV file parsed successfully! Configure column mapping below.",
      );
    };
  ...
  const getMatchedThanaId = (thanaName: string, districtId: number | null) => {
    ...
    let matched = thanas.find(
      (t) => t.name.toLowerCase() === thanaName.trim().toLowerCase(),
    );
    if (districtId) {
      const sameDistrictThana = thanas.find(
        (t) =>
          t.name.toLowerCase() === thanaName.trim().toLowerCase() &&
  ...
  const getMatchedTierId = (tierName: string) => {
    ...
    const matched = tiers.find(
      (t) => t.name.toLowerCase() === tierName.trim().toLowerCase(),
    );
    return matched ? matched.id : null;
  };
  ...
  const previewRows = parsedCsvData
    ? parsedCsvData.rows.map((row, rowIdx) => {
        ...
        const resolvedDistrictId =
          getMatchedDistrictId(csvDistrict) ||
          defaultSettings.district_id ||
          null;

        const csvThana = getMappedValue(row, columnMapping.thana);
        const resolvedThanaId =
          getMatchedThanaId(csvThana, resolvedDistrictId) ||
          defaultSettings.thana_id ||
          null;

        const csvTier = getMappedValue(row, columnMapping.membership_tier);
        const resolvedTierId =
          getMatchedTierId(csvTier) ||
          defaultSettings.membership_tier_id ||
          null;

        const csvActive = getMappedValue(row, columnMapping.is_active);
        const resolvedActive = csvActive
          ? ["yes", "true", "1", "active"].includes(csvActive.toLowerCase())
          : true;
        ...
        return {
          ...
          district_name:
            csvDistrict ||
            (resolvedDistrictId
              ? districts.find((d) => d.id === resolvedDistrictId)?.name
              : "-"),
          thana_id: resolvedThanaId,
          thana_name:
            csvThana ||
            (resolvedThanaId
              ? thanas.find((t) => t.id === resolvedThanaId)?.name
              : "-"),
          membership_tier_id: resolvedTierId,
          membership_tier_name:
            csvTier ||
            (resolvedTierId
              ? tiers.find((t) => t.id === resolvedTierId)?.name
              : "Regular"),
  ...
  const executeCrmBulkImport = async () => {
    ...
    for (let i = 0; i < previewRows.length; i++) {
      ...
      const payload = {
        ...
        gender: ["male", "female", "other"].includes(row.gender?.toLowerCase())
    ...
    message.success(
      `Bulk import completed: ${successCount} successfully registered, ${failCount} failed.`,
    );
  ...
  const handleSave = async (values: any) => {
    try {
      ...
      if (selectedCustomer) {
        await axios.put(
          `${CRM_URL}/customers/${selectedCustomer.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
  ...
  const handleToggleActive = async (record: any, checked: boolean) => {
    try {
      ...
      message.success(
        `User status changed to: ${checked ? "Active" : "Inactive (Soft-Disabled)"}`,
      );
      fetchCustomers();
    } catch (err: any) {
      if (err?.response?.status !== 403)
  ...
  const handleDelete = async (id: number) => {
    Modal.confirm({
      ...
      content:
        "Are you sure you want to delete this customer? This action is protected by referential rules if the customer has existing orders.",
      okText: "Delete",
      okType: "danger",
      onOk: async () => {
        try {
  ...
  const handleImpersonate = async (id: number) => {
    try {
      ...
      const res = await axios.post(
        `${CRM_URL}/customers/${id}/impersonate`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      message.success(
        `Impersonating customer ${res.data.impersonated_username} in a new tab.`,
      );
    } catch (err) {
      console.error(err);
  ...
  const handlePostNote = async (values: any) => {
    ...
    try {
      const token = localStorage.getItem("poshplex_access_token");
      const res = await axios.post(
        `${CRM_URL}/customers/${detailData.profile.id}/notes`,
        values,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleExportCSV = () => {
    ...
    csvContent +=
      "Name,Phone,Email,Gender,Birthdate,Membership Type,District,Thana,Address,Active,Orders,Total Spent\n";
  ...
  const customerColumns = [
    {
      ...
      render: (text: string, rec: any) => (
        <a onClick={() => fetchCustomerDetail(rec.id)}>
          <b>{text}</b>
        </a>
      ),
    },
    { title: "Phone number", dataIndex: "phone", key: "phone" },
    {
      ...
      render: (record: any) => (
        <span>
          <EnvironmentOutlined style={{ color: "var(--accent-purple)" }} />{" "}
          {record.thana_name || "Thana"}, {record.district_name || "District"}
        </span>
      ),
    },
    {
      ...
      render: (tier: string) => (
        <Tag color="purple">{tier?.toUpperCase() || "REGULAR"}</Tag>
      ),
    },
    {
      title: "Total Orders",
      ...
      key: "total_orders",
      render: (val: any) => <Badge count={val} showZero color="#8b5cf6" />,
    },
    {
      title: "Spend (BDT)",
      ...
      render: (record: any) => (
        <Space>
          <Button
            ...
            onClick={() => {
              ...
              if (record.district_id) {
                const matched = thanas.filter(
                  (t) => t.district_id === record.district_id,
                );
                setFilteredThanas(matched);
              }
          ...
          <Button
            ...
            style={{ backgroundColor: "var(--accent-cyan)", border: "none" }}
          >
            Impersonate
          </Button>
          <Popconfirm
    ...
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      ...
      >
        <div>
          ...
          >
            Manage customer profiles, shipping routes, and reviews.
          </p>
        </div>
      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            label: (
              <span>
                <SolutionOutlined /> Membership Tiers
              </span>
            ),
          ...
          },
          {
            ...
            label: (
              <span>
                <DownloadOutlined /> Bulk CSV Import
              </span>
            ),
            children: (
                  ...
                  <div>
                    ...
                    >
                      ...
                      <p className="ant-upload-text">
                        Click or drag CSV file here to start column mapping
                      </p>
                    </Upload.Dragger>
                  </div>
                ) : (
                  <div>
                    ...
                    >
                      ...
                      <Divider style={{ margin: "12px 0" }}>
                        Fallback Defaults (When Mappings Are Empty)
                      </Divider>
                    ...
                    <div style={{ marginBottom: 24 }}>
                      <h3 style={{ marginBottom: 12 }}>
                        Rearranged Preview (Showing Mapped Fields)
                      </h3>
                      <Table
                        scroll={{ x: "max-content" }}
                        size="small"
                        ...
                        columns={[
                          ...
                          },
                          {
                            ...
                            dataIndex: "gender",
                            key: "gender",
                            render: (val) => val || "unspecified",
                          },
                          {
                            title: "Mapped Birthdate",
                            dataIndex: "birthdate",
                            key: "birthdate",
                            render: (val) => val || "-",
                          },
                    ...
                    {importProgress.total > 0 && (
                      <div style={{ marginBottom: 24 }}>
                        ...
                        <p style={{ marginTop: 8 }}>
                          ...
                          {importProgress.total} profiles completed...
                        </p>
                      </div>
                    )}
        ...
        >
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item
                name="phone"
                label="Unique Phone Number (Identity)"
                rules={[{ required: true }]}
              >
                <Input
                  style={{ borderRadius: 0 }}
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item
                ...
                label="Membership tier Assignment"
              >
        ...
        {detailData && (
          <Space direction="vertical" size="large" style={{ width: "100%" }}>
            ...
            <Alert
              ...
              type={
                detailData.risk_profile.cod_reliability_score < 75
                  ? "error"
                  : detailData.risk_profile.cod_reliability_score < 90
                    ? "warning"
                    : "success"
              }
              showIcon
            ...
            <Descriptions title="Customer Profile" bordered column={2}>
              ...
              <Descriptions.Item label="Membership Tier">
                {detailData.profile.membership_tier_name || "Regular"}
              </Descriptions.Item>
              <Descriptions.Item label="Gender">
                {detailData.profile.gender?.toUpperCase()}
              </Descriptions.Item>
              <Descriptions.Item label="Birthday">
                {detailData.profile.birthdate || "-"}
              </Descriptions.Item>
              <Descriptions.Item label="District">
                {detailData.profile.district_name || "-"}
              </Descriptions.Item>
              <Descriptions.Item label="Thana">
                {detailData.profile.thana_name || "-"}
              </Descriptions.Item>
              <Descriptions.Item label="Detailed Address" span={2}>
            ...
            <Row gutter={16}>
              ...
              <Col span={6}>
                <Card size="small">
                  ...
                  >
                    {detailData.kpis.last_order_date
                      ? new Date(
                          detailData.kpis.last_order_date,
                        ).toLocaleDateString()
                      : "-"}
            ...
            <Form form={noteForm} onFinish={handlePostNote} layout="vertical">
              <Form.Item
                name="note"
                label="Record new timeline log remark"
                rules={[{ required: true }]}
              >
                <Input.TextArea
                  placeholder="Type internal remark notes here..."
                  rows={2}
                  style={{ borderRadius: 0 }}
                />
              </Form.Item>
            ...
            <Timeline style={{ marginTop: 24 }}>
              {detailData.notes?.map((n: any) => (
                <Timeline.Item key={n.id}>
                  <b>Log remark by {n.author_username}</b> -{" "}
                  <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
